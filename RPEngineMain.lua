...
local Passe = {
	Moto = {ID = "25245158", Tool = RSF:WaitForChild("Porte de Armas de Fogo")}
}
...
game.Players.PlayerAdded:Connect(function(Jogador)
	...
	local ModuloWL = require(game.ReplicatedStorage.RPEngine.Whitelist)
	
	local Succ,Err = pcall(function()
	for _,V in pairs(ModuloWL) do
		if not table.find(V, Jogador.Name) then
